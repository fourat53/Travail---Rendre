...
public class Magsin {
    ...
    public void ajouterProduit(Produit p){
        if (this.capacite<50){
            ...
            System.out.println("Tableau de produits plein-----");
        }
    }
     ...
     public boolean estFrais(String saison, Produit p){
         boolean b=false;
         if (p instanceof produitFruit){
            if (((produitFruit)p).saison==saison){
                b=true;
            }
        }
        else if (p instanceof produitLegume){
            if (((produitLegume)p).saison==saison){    //interface??
                b=true;
            }
        }return b;    
     }
